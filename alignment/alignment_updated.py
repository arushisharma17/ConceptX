...
def assign_labels_to_clusters_2(label_map, clusters, threshold):
    ...
    for cluster_id, cluster_words in g_c:
        for label_id, label_words in label_map.items():
            ...
            if match >= threshold and len(set(word_items)) > 5:
...
def analyze_clusters(dictionary, unique_labels, assigned_cluster_count, threshold, method, csv_writer):
    key_count = {}
    ...
    num_tags_covered = len(key_count)
    num_unique_tags = len(unique_labels)

    overall_alignment = ((assigned_cluster_count / 600) + (num_tags_covered / num_unique_tags)) / 2

    # Log results to CSV
    csv_writer.writerow([method, threshold, num_clusters, num_tags_covered, num_unique_tags, overall_alignment])
...
def main():
    ...
    with open('analysis_results.csv', mode='w', newline='') as csv_file:
        ...
        csv_writer.writerow(['Method', 'Threshold', 'Num Clusters', 'Num Tags Covered', 'Num Unique Tags', 'Overall Alignment'])
        ...
        for method in args.methods:
            for threshold in args.thresholds:
                ...
                analyze_clusters(assigned_clusters, unique_labels, assigned_cluster_count, threshold, method, csv_writer)
